...
class AsyncRootLayerClient:
    ...
    async def __aenter__(self) -> AsyncRootLayerClient:
        return self

    async def __aexit__(self, exc_type: object, exc: object, tb: object) -> None:  # noqa: ANN401
        await self.aclose()
    ...
    async def submit_intent(
        self,
        req: SubmitIntentRequest | Mapping[str, Any],
    ) -> SubmitIntentResponse:
        req_model = (
    ...
    async def submit_intent_batch(
        self, req: SubmitIntentBatchRequest | Mapping[str, Any]
    ) -> SubmitIntentBatchResponse:
        req_model = (
            req
    ...
    async def get_intent(self, intent_id: str) -> Intent:
        data = await self._request_json("GET", f"/api/v1/intents/query/{intent_id}")
    ...
    async def get_intents(
        ...
        data = await self._request_json("GET", "/api/v1/intents/query", params=params)
        return GetIntentsResponse.model_validate(data)

    async def post_assignment(self, req: Assignment | Mapping[str, Any]) -> Ack:
    ...
    async def post_assignment_batch(self, req: AssignmentBatch | Mapping[str, Any]) -> Ack:
    ...
    async def submit_direct_intent(
