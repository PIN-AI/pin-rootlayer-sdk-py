...
def test_e2e_submit_direct_intent() -> None:
    ...
    target_agent_id = _env("TARGET_AGENT_ID")
    ...
    if not all(
        ...
        pytest.skip(
            ...
            "TARGET_AGENT_ID, INTENT_MANAGER_ADDRESS, CHAIN_ID)",
        )
    ...
    assert resp.ok is True or (resp.status or "").upper() in {"DISPATCHED", "AGENT_OFFLINE"}
